...
public class IntegrationProcessorInRiverChannelNode : IntegrationProcessorInRiver
{
    public IntegrationProcessorInRiverChannelNode(IntegrationProcessorFileHelper fileHelper)
        : base(fileHelper) { }
            ...
            if (!string.IsNullOrEmpty(categoryName))
            {
                fields.AddRange(
                    new List<string>
                    {
                        collectionObject.UniqueIdentifier,
                        categoryName,
                        collectionObject.Action == "Deleted" ? "true" : "false"
                    }
                );

                var channelNodeFields = fields.Select(o => o.Clone()).ToList();
                integrationTable.LoadDataRow(channelNodeFields.ToArray(), true);
            }
            else
            {
