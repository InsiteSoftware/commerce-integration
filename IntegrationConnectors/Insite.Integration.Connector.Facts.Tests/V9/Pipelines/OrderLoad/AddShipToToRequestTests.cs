...
[TestFixture]
public class AddShipToToRequestTests : BaseForPipeTests<OrderLoadParameter, OrderLoadResult>
{
    ...
    [Test]
    public void Execute_Should_Populate_ShipToNumber_With_TEMP_When_ShipTo_Is_OneTimeShipTo()
    {
        ...
        Assert.AreEqual(
            "TEMP",
            result.OrderLoadRequest.Request.Orders.First().OrderHeader.ShipToNumber
        );
    }
    ...
    [Test]
    public void Execute_Should_Populate_Name_With_Warehouse_Name_When_FulfillmentMethod_Is_Pickup()
    {
        var customerOrder = Some.CustomerOrder()
        ...
        Assert.AreEqual(
            ...
            result.OrderLoadRequest.Request.Orders.First().OrderHeader.ShipToName
        );
    }

    [Test]
    public void Execute_Should_Populate_Name_With_STCompanyName_When_FulfillmentMethod_Is_Not_Pickup()
    {
        var customerOrder = Some.CustomerOrder()
            .WithFulfillmentMethod(FulfillmentMethod.Ship.ToString())
            .WithShipTo(Some.Customer())
            .WithSTCompanyName("TheCompany")
            .Build();
    ...
    public void Execute_Should_Populate_ShipTo_Properties_From_CustomerOrder()
    {
        ...
        Assert.AreEqual(
            ...
            result.OrderLoadRequest.Request.Orders.First().OrderHeader.ShipToAddress1
        );
        Assert.AreEqual(
            customerOrder.STAddress2,
            result.OrderLoadRequest.Request.Orders.First().OrderHeader.ShipToAddress2
        );
        Assert.AreEqual(
            customerOrder.STAddress3,
            result.OrderLoadRequest.Request.Orders.First().OrderHeader.ShipToAddress3
        );
        Assert.AreEqual(
            customerOrder.STAddress4,
            result.OrderLoadRequest.Request.Orders.First().OrderHeader.ShipToAddress4
        );
        Assert.AreEqual(
            customerOrder.STCity,
            result.OrderLoadRequest.Request.Orders.First().OrderHeader.ShipToCity
        );
        Assert.AreEqual(
            customerOrder.STPostalCode,
            result.OrderLoadRequest.Request.Orders.First().OrderHeader.ShipToZipCode
        );
    }
    ...
    public void Execute_Should_Populate_State_And_Country_With_State_And_Country_Abbreviation_When_State_And_Country_Exist_In_Repository()
    {
        ...
        Assert.AreEqual(
            state.Abbreviation,
            result.OrderLoadRequest.Request.Orders.First().OrderHeader.ShipToState
        );
        Assert.AreEqual(
            country.Abbreviation,
            result.OrderLoadRequest.Request.Orders.First().OrderHeader.ShipToCountry
        );
    }
