    ...
    : BaseForPipeTests<SFOEOrderTotLoadV4Parameter, SFOEOrderTotLoadV4Result>
{
    ...
    public void Execute_Should_Populate_A_InHeaderExtraData2_For_ShippingCharges()
    {
        ...
        this.WhenGetShippingDiscountTotalIs(customerOrder, 10);
        var parameter = this.GetDefaultParameter();
        parameter.CustomerOrder = customerOrder;

        var result = this.RunExecute(parameter);

        var expectedFieldValue = $"addonno=2\taddonamt=4\taddontype=$";

        Assert.AreEqual(
            expectedFieldValue,
            result.SFOEOrderTotLoadV4Request.Request.InputHeaderExtraDataCollection.InputHeaderExtraDatas
    ...
    public void Execute_Should_Not_Populate_Discount_Amount_When_Discount_Amount_Is_Less_Than_Or_Equal_To_Zero()
    {
        ...
        Assert.IsFalse(
            result.SFOEOrderTotLoadV4Request.Request.InputHeaderExtraDataCollection.InputHeaderExtraDatas.Any(
    ...
    private void WhenGetShippingDiscountTotalIs(
        CustomerOrder customerOrder,
        decimal shippingDiscountTotal
    )
    {
        this.customerOrderUtilities
            .Setup(o => o.GetPromotionShippingDiscountTotal(customerOrder))
            .Returns(shippingDiscountTotal);
    }
}
